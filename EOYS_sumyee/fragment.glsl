...
float raymarchwater(vec3 camera, vec3 start, vec3 end, float depth){
    ...
    float eps = 0.01;
    for(int i=0;i<318;i++){
        h = getwaves(pos.xz * 0.1, u_RayMarch) * depth - depth;
...
vec3 normal(vec2 pos, float e, float depth){
    ...
    return (cross(normalize(a-vec3(pos.x - e, getwaves((pos.xy - ex.xy)*0.1, u_Normal) * depth, pos.y)), 
                           normalize(a-vec3(pos.x, getwaves((pos.xy + ex.yx )* 0.1, u_Normal) * depth, pos.y + e))));
}
